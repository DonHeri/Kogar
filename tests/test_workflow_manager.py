...
@pytest.fixture
def wm(household: Household) -> WorkflowManager:
...
def test_workflow_manager_starts_in_planning_phase(wm: WorkflowManager) -> None:
    """El ciclo empieza en PLANNING: registrar ya no es una fase aparte"""
    assert wm.current_phase == Phase.PLANNING


# ====================================================
# TESTS: PLANNING PHASE - register_member
# ====================================================
...
def test_register_member_wrong_phase(wm_in_month: WorkflowManager) -> None:
...
def test_set_income_valid(wm: WorkflowManager) -> None:
...
def test_set_income_wrong_phase(wm_in_month: WorkflowManager) -> None:
...
# ====================================================
# TESTS: PLANNING - miembros, ingresos y congelado del acuerdo
# ====================================================


def test_members_and_incomes_are_editable_during_planning(wm: WorkflowManager) -> None:
...
def test_income_change_recalculates_split_while_planning(wm: WorkflowManager) -> None:
...
def test_finish_planning_requires_members(wm: WorkflowManager) -> None:
...
def test_finish_planning_requires_incomes(wm: WorkflowManager) -> None:
...
def test_finish_planning_freezes_the_agreement(wm: WorkflowManager) -> None:
    ...
    agreed = wm.get_agreed_percentages()

    assert agreed == {"amanda": 6000, "heri": 4000}


def test_partial_incomes_are_valid(wm: WorkflowManager) -> None:
...
def test_get_registered_members_empty(wm: WorkflowManager) -> None:
    """get_registered_members retorna lista vacía si no hay miembros"""
    assert wm.get_registered_members() == []


def test_get_registered_members_multiple(wm: WorkflowManager) -> None:
...
def test_get_member_income_nonexistent(wm: WorkflowManager) -> None:
    """get_member_income con nombre inexistente lanza ValueError"""
    with pytest.raises(ValueError, match="does not exist"):
        wm.get_member_income("Nadie")


def test_get_member_income_after_planning(wm: WorkflowManager) -> None:
...
def test_get_total_incomes_empty(wm: WorkflowManager) -> None:
...
def test_get_total_incomes_multiple_members(wm: WorkflowManager) -> None:
...
def test_set_budget_for_category_in_planning_phase(
...
def test_set_budget_for_category_raises_if_not_in_planning(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_add_category_with_parent_in_planning(wm_in_planning: WorkflowManager) -> None:
...
def test_finish_planning_raises_if_not_in_planning(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_finish_planning_raises_if_no_budget_assigned(wm: WorkflowManager) -> None:
...
def test_add_category_in_planning_phase(wm_in_planning: WorkflowManager) -> None:
...
def test_add_category_raises_if_not_in_planning(wm_in_month: WorkflowManager) -> None:
    """add_category() lanza error si no estamos en PLANNING"""
    with pytest.raises(ValueError, match="planning"):
        wm_in_month.add_category("educacion")


def test_set_standard_categories_creates_defaults(wm: WorkflowManager) -> None:
...
def test_remove_category_in_planning_phase(wm_in_planning: WorkflowManager) -> None:
...
def test_remove_category_raises_if_not_in_planning(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_get_incomes_returns_live_values(wm: WorkflowManager) -> None:
...
def test_get_agreed_percentages_fails_in_planning(wm: WorkflowManager) -> None:
...
def test_set_custom_splits_in_planning_phase(wm: WorkflowManager) -> None:
...
def test_set_custom_splits_raises_if_not_in_planning(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_register_expense_explicit_is_shared_overrides_behavior(
    wm: WorkflowManager,
) -> None:
...
def test_get_month_summary_raises_if_not_in_month(wm: WorkflowManager) -> None:
...
def test_set_budget_by_percentages_partial_allocation_raises(
    wm: WorkflowManager,
) -> None:
...
def test_set_budget_by_percentages_wrong_phase(wm_in_month: WorkflowManager) -> None:
...
def test_finish_month_transitions_to_closing(wm_in_month: WorkflowManager) -> None:
...
def test_finish_month_adds_closing_to_completed_phases(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_finish_month_raises_if_not_in_month(wm: WorkflowManager) -> None:
    """finish_month lanza ValueError si no estamos en MONTH"""
    with pytest.raises(ValueError):
        wm.finish_month()


def test_get_settlement_accessible_after_finish_month(
    wm_in_month: WorkflowManager,
) -> None:
...
def test_deposit_to_bucket_increases_balance(
    wm_in_month_two_members: tuple[WorkflowManager, UUID],
) -> None:
...
def test_withdraw_from_bucket_reduces_balance(
    wm_in_month_two_members: tuple[WorkflowManager, UUID],
) -> None:
...
def test_get_all_buckets_returns_all(
    wm_in_month_two_members: tuple[WorkflowManager, UUID],
) -> None:
...
def test_get_buckets_by_member_filters_correctly(
    wm_in_month_two_members: tuple[WorkflowManager, UUID],
) -> None:
...
def test_deposit_outside_month_raises(wm: WorkflowManager) -> None:
...
def test_withdraw_outside_month_raises(wm: WorkflowManager) -> None:
...
def test_full_flow_registration_to_closing(wm: WorkflowManager) -> None:
    ...
    # Miembros e ingresos, ya dentro de PLANNING
    wm.register_member("Amanda")
    wm.register_member("Heri")
...
def test_start_new_month_returns_to_planning_phase(
    wm_in_month_two_members: tuple[WorkflowManager, UUID],
) -> None:
...
def test_last_payments_dont_appear_in_new_month(wm: WorkflowManager) -> None:
...
def test_payment_on_cut_off_day_counts_only_in_the_month_that_starts(
    wm: WorkflowManager,
) -> None:
...
def test_new_period_starts_today_when_no_date_given(wm: WorkflowManager) -> None:
...
def test_payment_before_period_start_is_rejected(
    wm_month_from_28_jan: tuple[WorkflowManager, UUID],
) -> None:
...
def test_payment_on_period_start_day_is_accepted(
    wm_month_from_28_jan: tuple[WorkflowManager, UUID],
) -> None:
...
def test_saving_deposit_before_period_start_is_rejected(
    wm_month_from_28_jan: tuple[WorkflowManager, UUID],
) -> None:
