...
def test_creation_defaults_start_date_to_today(car_bucket: DebtBucket) -> None:
    assert car_bucket.start_date.date() == datetime.today().date()


def test_creation_with_explicit_start_date() -> None:
    start = datetime(2025, 1, 1)
    bucket = DebtBucket("B", 100000, "heri", 10000, start_date=start)
    assert bucket.start_date == start


def test_id_is_generated(car_bucket: DebtBucket) -> None:
...
def test_installment_is_mandatory() -> None:
...
def test_empty_owner_raises() -> None:
    with pytest.raises(ValueError, match="owner"):
        DebtBucket("B", 100000, "", 10000)


def test_principal_cents_zero_raises() -> None:
    with pytest.raises(ValueError):
        DebtBucket("B", 0, "heri", 10000)


def test_principal_cents_negative_raises() -> None:
    with pytest.raises(ValueError):
        DebtBucket("B", -100, "heri", 10000)


def test_principal_cents_float_raises() -> None:
    principal: object = 100.0
    with pytest.raises(TypeError):
        DebtBucket("B", principal, "heri", 10000)  # type: ignore[arg-type]


def test_principal_cents_bool_raises() -> None:
    with pytest.raises(TypeError):
        DebtBucket("B", True, "heri", 10000)


def test_installment_cents_zero_raises() -> None:
    with pytest.raises(ValueError):
        DebtBucket("B", 100000, "heri", 0)


def test_installment_cents_negative_raises() -> None:
    with pytest.raises(ValueError):
...
def test_pay_increases_total_paid(car_bucket: DebtBucket) -> None:
    car_bucket.pay(50000, "heri")
    assert car_bucket.total_paid == 50000


def test_pay_accumulates(car_bucket: DebtBucket) -> None:
    car_bucket.pay(30000, "heri")
    car_bucket.pay(20000, "heri")
    assert car_bucket.total_paid == 50000


def test_pay_decreases_remaining_balance(car_bucket: DebtBucket) -> None:
    car_bucket.pay(50000, "heri")
    assert car_bucket.remaining_balance == 2000000 - 50000


def test_pay_with_date(car_bucket: DebtBucket) -> None:
    date = datetime(2026, 3, 1)
    car_bucket.pay(50000, "heri", date=date)
    assert car_bucket._entries[0].date == date


def test_pay_raises_if_member_not_owner(car_bucket: DebtBucket) -> None:
    with pytest.raises(ValueError, match="pertenece"):
        car_bucket.pay(50000, "amanda")


def test_pay_invalid_amount_zero_raises(car_bucket: DebtBucket) -> None:
    with pytest.raises(ValueError):
        car_bucket.pay(0, "heri")


def test_pay_invalid_amount_negative_raises(car_bucket: DebtBucket) -> None:
    with pytest.raises(ValueError):
        car_bucket.pay(-100, "heri")


def test_pay_overpayment_is_allowed(car_bucket: DebtBucket) -> None:
...
def test_advancing_money_reduces_remaining_installments(car_bucket: DebtBucket) -> None:
...
def test_remove_payment_removes_entry(car_bucket: DebtBucket) -> None:
    car_bucket.pay(30000, "heri")
    car_bucket.pay(20000, "heri")
...
def test_remove_payment_raises_if_id_not_found(car_bucket: DebtBucket) -> None:
...
def test_is_closed_false_when_balance_remains(car_bucket: DebtBucket) -> None:
    car_bucket.pay(1999999, "heri")
    assert car_bucket.is_closed is False


def test_is_closed_true_when_balance_reaches_zero(car_bucket: DebtBucket) -> None:
...
def test_set_installment_overrides_previous(car_bucket: DebtBucket) -> None:
    car_bucket.set_installment(120000)
    assert car_bucket.installment_cents == 120000


def test_set_installment_invalid_raises(car_bucket: DebtBucket) -> None:
    with pytest.raises(ValueError):
...
def test_next_installment_equals_user_installment(car_bucket: DebtBucket) -> None:
    assert car_bucket.next_installment == 100000


def test_next_installment_capped_at_remaining_on_last_payment(
    car_bucket: DebtBucket,
) -> None:
...
def test_remaining_installments_at_start(car_bucket: DebtBucket) -> None:
    assert car_bucket.remaining_installments == 20  # 2000000 / 100000


def test_remaining_installments_after_partial_payment(car_bucket: DebtBucket) -> None:
...
def test_remaining_installments_zero_when_closed(car_bucket: DebtBucket) -> None:
    car_bucket.pay(2000000, "heri")
