...
def test_create_valid_member(base_member: Member) -> None:
...
def test_create_member_empty_name_raises_error() -> None:
    """Lanza error si nombre está vacío"""
    with pytest.raises(ValueError, match="Nombre no puede estar vacío"):
        Member("")


def test_create_member_whitespace_name_raises_error() -> None:
...
def test_add_incomes_updates_correctly(base_member: Member) -> None:
...
def test_add_incomes_zero_is_valid(base_member: Member) -> None:
...
def test_add_negative_income_raises_error(base_member: Member) -> None:
