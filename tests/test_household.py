...
def household_with_members_and_child_categories(household_with_members):
    household_with_members.add_category("")
    household_budget = household_with_members.budget

    household_budget.add_category("vivienda", parent="fijos")
    household_budget.add_category("suministros", parent="fijos")
...
def test_set_budget_for_child_category(household_with_members):
    household_with_members.set_budget_for_category("fijos", amount_cents=40000)

    household_with_members.add_category("vivienda", parent="fijos")
    household_with_members.set_budget_for_category(
        category="vivienda", amount_cents=30000
    )

    assert household_with_members.budget.categories["vivienda"].planned_amount == 30000
...
def test_get_over_ceiling_parent_category_raises_error(
    household_with_members_and_child_categories,
):
    household_with_members_and_child_categories.set_budget("fijos", 50000)
    household_with_members_and_child_categories.set_budget("vivienda", 30000)

    with pytest.raises(
        ValueError, match="No se puede superar el techo de la categoría raíz"
    ):
        household_with_members_and_child_categories.set_budget("suministros", 30000)
...
def test_get_settlement_three_members_only_should_pay_two(base_household):
    """3 miembros EQUAL: uno paga todo → los otros dos le deben a partes iguales"""
    m1, m2, m3 = Member("alice"), Member("bob"), Member("carol")
    for m in (m1, m2, m3):
        m.monthly_income = 100000
